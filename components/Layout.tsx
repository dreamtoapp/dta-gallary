...
export default function Layout({ children }: LayoutProps) {
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!userId) return;
    // Simple prefix logic: C = client, D = designer, A = admin
    if (userId.startsWith("C")) {
      router.push(`/client/${userId}`);
    } else if (userId.startsWith("D")) {
      router.push(`/designer/${userId}`);
    } else if (userId.startsWith("A")) {
      router.push(`/admin/clients`); // or `/admin/${userId}` if you have per-admin pages
    } else {
      alert("معرف غير صالح. الرجاء إدخال معرف صحيح.");
    }
  }
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <nav dir="rtl" className="w-full flex items-center justify-between px-4 md:px-8 py-3 border-b bg-white shadow-sm">
        ...
        {/* Compact Login Form */}
        <form onSubmit={handleSubmit} className="flex items-center gap-2 w-full max-w-xs md:max-w-sm">
          <input
            id="user-id-input"
            type="text"
            value={userId}
